...
ICFaces = {};
ICFaces.AjaxUpload = PrimeFaces.widget.BaseWidget.extend({
    init: function (a, file) {
        // Convert new parameter
        this._super(a);
        var begin = 'data:' + this.cfg.cropper.file.type + ';base64,';
        var blob = this.cfg.base64Str;
        blob = blob.replace(begin,'');
        this.file = this.cfg.cropper.file;
    },
    getType: function () {
    ...
    doUpload: function () {
        ...
        formData.append("file", this.file); //this.dataURItoBlob(this.file)
        formData.append("upload_file", true);

        $.ajax({
            type: "POST",
            url: '/ISM/faces/upload',
            xhr: function () {
    ...
    dataURItoBlob: function (dataURI_) {
        // convert base64 to raw binary data held in a string
        // doesn't handle URLEncoded DataURIs - see SO answer #6850276 for code that does this
        dataUri = dataURI_ + '';
        
        var byteString = atob(dataUri); //atob(dataURI.split(',')[1]);

        // separate out the mime component
        var mimeString = this.cfg.cropper.file.type; // dataURI.split(',')[0].split(':')[1].split(';')[0];
...
//                fd.append("file", a, 'croppedFile');
//                fd.append("upload_file", true);
//                var xhr = new XMLHttpRequest();
//                xhr.open('POST', '/ISM/faces/upload', true);
//                xhr.send(fd);

                var file = a;//$(this)[0].files[0];
                var upload = new ICFaces.AjaxUpload(this, file.blob);
