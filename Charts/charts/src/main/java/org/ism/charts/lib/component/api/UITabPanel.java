...
public class UITabPanel extends UIPanel implements NamingContainer {
    ...
    /**
     * Handle case where this table is nested inside another table. See method
     * getDataModel for more details.
     * <p>
     * Key: parentClientId (aka rowId when nested within a parent table) Value:
     * DataModel
     */
     ...
     * previously saved by method saveDescendantComponentStates.
     * <p>
     * The saved state info only covers those fields that are expected to vary
     * between rows of a table. Other fields are not modified.
     */
     ...
     * when the view is serialized. Transient components are skipped (no state
     * is saved for them).
     * <p>
     * If there are no children then null is returned. If there are one or more
     * children, and all children are transient then an empty collection is
     * returned; this will happen whenever a table contains only read-only
     * components.
     * <p>
     * Otherwise a collection is returned which contains an object for every
     * non-transient child component; that object may itself contain a
     * collection of the state of that child's child components.
     */
