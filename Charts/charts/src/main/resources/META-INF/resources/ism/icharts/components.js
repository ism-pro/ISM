        ...
        openDialog: function (e) {
            ...
            c.on("load", function () {
                ...
                if (e.options.headerElement) {
                    var n = IChartsFaces.escapeClientId(e.options.headerElement), k = c.contents().find(n);
                    if (k.length) {
                        l = k;
                        o = true;
                    }
                ...
                if (!p.data("initialized")) {
                    IChartsFaces.cw.call(g.IChartsFaces, "DynamicDialog", i, {id: j, position: "center", sourceComponentId: e.sourceComponentId, sourceWidgetVar: e.sourceWidgetVar, onHide: function () {
                            var s = this, r = this.content.children("iframe");
                            if (r.get(0).contentWindow.IChartsFaces) {
                                this.destroyIntervalId = setInterval(function () {
                                    if (r.get(0).contentWindow.IChartsFaces.ajax.Queue.isEmpty()) {
                ...
                if (e.options.contentHeight) {
                    ...
                    m = p.get(0).contentWindow.document.body.scrollHeight + (IChartsFaces.env.browser.webkit ? 5 : 25)
                }
                p.css("height", m)
            }).attr("src", a)
...
IChartsFaces.widget.AutoComplete = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        ...
        this.input.on("keyup.autoComplete", function (f) {
            ...
            if (IChartsFaces.isIE(9) && b === d.BACKSPACE) {
                a.processKeyEvent(f)
            }
        ...
        if (IChartsFaces.isIE(8) && (c.itemClick || d.originalEvent.propertyName !== "value")) {
            c.itemClick = false;
            return
        }
        var b = c.input.val();
...
IChartsFaces.widget.BlockUI = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        ...
        for (var b = 0; b < this.block.length; b++) {
            ...
            c.css({left: (a.width() - c.outerWidth()) / 2, top: (a.height() - c.outerHeight()) / 2, "z-index": ++IChartsFaces.zindex})
        }
        if (this.cfg.animate) {
            this.blocker.fadeIn()
        } else {
        ...
        if (this.block.length > 1) {
            this.blocker = $(IChartsFaces.escapeClientId(this.id + "_blocker"));
...
IChartsFaces.widget.Calendar = IChartsFaces.widget.BaseWidget.extend({init: function (b) {
        ...
        if (this.cfg.popup && this.cfg.showOn) {
            ...
            e.attr("aria-label", IChartsFaces.getAriaLabel("calendar.BUTTON")).attr("aria-haspopup", true).html("").addClass("ui-button ui-widget ui-state-default ui-corner-all ui-button-icon-only").append('<span class="ui-button-icon-left ui-icon ui-icon-calendar"></span><span class="ui-button-text">ui-button</span>');
...
IChartsFaces.widget.ColumnToggler = IChartsFaces.widget.DeferredWidget.extend({init: function (a) {
        ...
        this.closer = $('<a href="#" class="ui-columntoggler-close"><span class="ui-icon ui-icon-close"></span></a>').attr("aria-label", IChartsFaces.getAriaLabel("columntoggler.CLOSE")).prependTo(this.panel);
        ...
        $(IChartsFaces.escapeClientId(d.attr("id") + "_clone")).removeClass("ui-helper-hidden");
        ...
        $(IChartsFaces.escapeClientId(d.attr("id") + "_clone")).addClass("ui-helper-hidden");
...
IChartsFaces.widget.DataTable = IChartsFaces.widget.DeferredWidget.extend({SORT_ORDER: {ASCENDING: 1, DESCENDING: -1, UNSORTED: 0}, init: function (a) {
        this._super(a);
        ...
        if (this.cfg.clientCache) {
            ...
            var b = {first: this.paginator.getFirst(), rows: this.paginator.getRows(), page: this.paginator.getCurrentPage()};
            this.clearCacheMap();
        ...
        this.ascMessage = IChartsFaces.getAriaLabel("datatable.sort.ASC");
        this.descMessage = IChartsFaces.getAriaLabel("datatable.sort.DESC");
        ...
        this.sortableColumns.on("mouseenter.dataTable", function () {
            ...
            if (f.cfg.scrollable) {
                $(IChartsFaces.escapeClientId(k.attr("id") + "_clone")).trigger("focus")
            }
            f.updateReflowDD(k, i)
        });
        if (this.reflowDD && this.cfg.reflow) {
            IChartsFaces.skinSelect(this.reflowDD);
        ...
        a.children(".ui-column-filter").each(function () {
            var c = $(this);
            if (c.is("input:text")) {
                IChartsFaces.skinInput(c);
                b.bindTextFilter(c)
            } else {
                IChartsFaces.skinSelect(c);
        ...
        this.getFocusableTbody().on("focus", function (b) {
            if (!a.mousedownOnRow) {
                ...
                if (a.cfg.scrollable) {
                    IChartsFaces.scrollInView(a.scrollBody, a.focusedRow)
                }
            }
        }).on("blur", function () {
            if (a.focusedRow) {
        ...
        this.scrollBody.on("scroll.dataTable", function () {
            ...
            if (c.cfg.virtualScroll) {
                ...
                c.scrollTimeout = setTimeout(function () {
                    var l = c.scrollBody.outerHeight(), k = c.bodyTable.outerHeight(), n = c.rowHeight * c.cfg.scrollStep, j = parseFloat((c.cfg.scrollLimit * c.rowHeight) + "px"), m = (j / n) || 1;
                ...
                if (c.shouldLiveScroll) {
                    var h = Math.ceil(this.scrollTop), g = this.scrollHeight, e = this.clientHeight;
        ...
        if (this.sortableColumns.length) {
            ...
            a.on("blur.dataTable", function () {
                ...
                if ((b === c.ENTER || b === c.NUMPAD_ENTER) && $(d.target).is(":not(:input)")) {
                    $(IChartsFaces.escapeClientId($(this).data("original"))).trigger("click.dataTable", (d.metaKey || d.ctrlKey));
                    d.preventDefault()
                }
            })
        }
        ...
        var c = this, d = (b - 1) * this.cfg.scrollStep, a = {source: this.id, process: this.id, update: this.id, formId: this.cfg.formId, params: [{name: this.id + "_scrolling", value: true}, {name: this.id + "_skipChildren", value: true}, {name: this.id + "_first", value: d}, {name: this.id + "_encodeFeature", value: true}], onsuccess: function (g, e, f) {
                IChartsFaces.ajax.Response.handle(g, e, f, {widget: c, handle: function (h) {
                        this.updateData(h);
        ...
        var b = d.rows, e = d.first, c = this, a = {source: this.id, process: this.id, update: this.id, global: false, params: [{name: this.id + "_skipChildren", value: true}, {name: this.id + "_encodeFeature", value: true}, {name: this.id + "_first", value: e}, {name: this.id + "_rows", value: b}, {name: this.id + "_pagination", value: true}, {name: this.id + "_clientCache", value: true}], onsuccess: function (h, f, g) {
                IChartsFaces.ajax.Response.handle(h, f, g, {widget: c, handle: function (j) {
                        if (j.length) {
                            var i = e + b;
                            c.cacheMap[i] = j
                        }
        ...
        var b = this, a = {source: this.id, process: this.id, update: this.id, global: false, params: [{name: this.id + "_pagination", value: true}, {name: this.id + "_encodeFeature", value: true}, {name: this.id + "_pageState", value: true}, {name: this.id + "_first", value: c.first}, {name: this.id + "_rows", value: c.rows}], onsuccess: function (f, d, e) {
        ...
        var e = this, b = {source: this.id, update: this.id, process: this.id, params: [{name: this.id + "_sorting", value: true}, {name: this.id + "_skipChildren", value: true}, {name: this.id + "_encodeFeature", value: true}], onsuccess: function (i, g, h) {
                IChartsFaces.ajax.Response.handle(i, g, h, {widget: e, handle: function (j) {
                        this.updateData(j)
                    }});
                return true
                ...
                if (!m.validationFailed) {
                    ...
                    if (!f) {
                        for (var k = 0; k < g.length; k++) {
                            ...
                            $(IChartsFaces.escapeClientId(h.attr("id") + "_clone")).removeAttr("aria-sort").attr("aria-label", e.getSortMessage(n, e.ascMessage))
                        }
                    ...
                    if (a === e.SORT_ORDER.DESCENDING) {
                        ...
                        $(IChartsFaces.escapeClientId(d.attr("id") + "_clone")).attr("aria-sort", "descending").attr("aria-label", e.getSortMessage(o, e.ascMessage))
                    } else {
                        if (a === e.SORT_ORDER.ASCENDING) {
                            ...
                            $(IChartsFaces.escapeClientId(d.attr("id") + "_clone")).attr("aria-sort", "ascending").attr("aria-label", e.getSortMessage(o, e.descMessage))
                        }
                    }
                }
        ...
        var c = this, a = {source: this.id, update: this.id, process: this.id, formId: this.cfg.formId, params: [{name: this.id + "_filtering", value: true}, {name: this.id + "_encodeFeature", value: true}], onsuccess: function (f, d, e) {
                ...
                if (c.cfg.virtualScroll) {
                    ...
                    if (f) {
                        ...
                        c.scrollBody.children("div").css({height: parseFloat((c.cfg.scrollLimit * c.rowHeight + 1) + "px")})
                    }
                } else {
                    if (c.cfg.liveScroll) {
                        c.scrollOffset = 0;
        ...
        var c = this, e = this.getRowMeta(d).index, a = {source: this.id, process: this.id, update: this.id, formId: this.cfg.formId, params: [{name: this.id + "_rowExpansion", value: true}, {name: this.id + "_expandedRowIndex", value: e}, {name: this.id + "_encodeFeature", value: true}, {name: this.id + "_skipChildren", value: true}], onsuccess: function (h, f, g) {
                IChartsFaces.ajax.Response.handle(h, f, g, {widget: c, handle: function (i) {
                        if (i && $.trim(i).length) {
        ...
        var g = this.getRowMeta(a.closest("tr")), e = a.children(".ui-cell-editor"), d = a.index(), f = this;
        ...
        var b = {source: this.id, process: this.id, update: this.id, global: false, params: [{name: this.id + "_encodeFeature", value: true}, {name: this.id + "_cellEditInit", value: true}, {name: this.id + "_cellInfo", value: c}], onsuccess: function (j, h, i) {
                IChartsFaces.ajax.Response.handle(j, h, i, {widget: f, handle: function (k) {
        ...
        this.currentCell = h;
        var b = h.children("div.ui-cell-editor"), a = b.children("div.ui-cell-editor-output"), j = b.children("div.ui-cell-editor-input"), d = j.find(":input:enabled"), e = d.length > 1;
        ...
        if (!h.data("edit-events-bound")) {
            ...
            d.on("keydown.datatable-cell", function (n) {
                var m = $.ui.keyCode, l = n.shiftKey, k = n.which, i = $(this);
        ...
        var h = this.getRowMeta(a.closest("tr")), e = a.children(".ui-cell-editor"), f = e.attr("id"), d = a.index(), g = this;
        if (this.cfg.scrollable && this.cfg.frozenColumns) {
        ...
        var b = {source: this.id, process: this.id, update: this.id, params: [{name: this.id + "_encodeFeature", value: true}, {name: this.id + "_cellInfo", value: c}, {name: f, value: f}], onsuccess: function (k, i, j) {
                IChartsFaces.ajax.Response.handle(k, i, j, {widget: g, handle: function (l) {
        ...
        var f = a.closest("tr"), g = this.getRowMeta(f).index, b = f.hasClass("ui-expanded-row"), e = this, c = {source: this.id, process: this.id, update: this.id, formId: this.cfg.formId, params: [{name: this.id + "_rowEditIndex", value: this.getRowMeta(f).index}, {name: this.id + "_rowEditAction", value: d}, {name: this.id + "_encodeFeature", value: true}], onsuccess: function (j, h, i) {
                IChartsFaces.ajax.Response.handle(j, h, i, {widget: e, handle: function (k) {
                        if (b) {
                            this.collapseRow(f)
                        }
                        this.updateRow(f, k)
        ...
        if (d === "save" && this.hasBehavior("rowEdit")) {
            ...
            if (d === "cancel" && this.hasBehavior("rowEditCancel")) {
                ...
                IChartsFaces.ajax.Request.handle(c)
            }
        }
        ...
        if ((e > a && g > a) || (o && e > a)) {
            ...
            if (this.cfg.scrollable) {
                var j = this.theadClone.parent(), n = d.index();
                if (o) {
                    var i = this;
                    j.width(j.width() + k);
                    ...
                    setTimeout(function () {
                        if (i.hasColumnGroup) {
                            ...
                            i.theadClone.find(IChartsFaces.escapeClientId(d.attr("id") + "_clone")).width(e);
                    ...
                    if (this.hasColumnGroup) {
                        ...
                        this.theadClone.find(IChartsFaces.escapeClientId(d.attr("id") + "_clone")).width(e);
                        this.theadClone.find(IChartsFaces.escapeClientId(f.attr("id") + "_clone")).width(g);
                        if (this.footerCols.length > 0) {
                            var h = this.footerCols.eq(n), c = h.next();
                            h.width(e);
                            c.width(g)
                        }
                    }
        ...
        return IChartsFaces.inArray(this.selection, a)
        ...
        $(this.jqId + " thead th").draggable({appendTo: "body", opacity: 0.75, cursor: "move", scope: this.id, cancel: ":input,.ui-column-resizer", start: function (b, c) {
                c.helper.css("z-index", ++IChartsFaces.zindex)
            }, drag: function (e, g) {
        ...
        this.tbody.sortable({placeholder: "ui-datatable-rowordering ui-state-active", cursor: "move", handle: "td,span:not(.ui-c)", appendTo: document.body, start: function (b, c) {
                ...
                if (a.cfg.scrollable) {
                    IChartsFaces.scrollInView(a.scrollBody, c.placeholder)
                }
            }})
        ...
        this.stickyContainer.css({position: "absolute", width: b.outerWidth(), top: f.top, left: f.left, "z-index": ++IChartsFaces.zindex});
        ...
        var d = e.first, c = e.rows + d, b = this.cfg.paginator.pageCount * e.rows, f = (!this.cacheMap[c]) && c < b;
        ...
        if (this.reflowDD && this.cfg.reflow) {
            var a = this.reflowDD.children("option"), b = c > 0 ? 0 : 1;
        ...
        var b = this.tbody.children("tr.ui-rowgroup-header"), a = b.find("> td:first > a.ui-rowgroup-toggler");
        a.off("click.dataTable-rowgrouptoggler").on("click.dataTable-rowgrouptoggler", function (g) {
            var d = $(this), c = d.children(".ui-rowgroup-toggler-icon"), f = d.closest("tr.ui-rowgroup-header");
...
IChartsFaces.widget.FrozenDataTable = IChartsFaces.widget.DataTable.extend({setupScrolling: function () {
        ...
        this.scrollBody.scroll(function () {
            var i = c.scrollBody.scrollLeft(), h = c.scrollBody.scrollTop();
            c.scrollHeaderBox.css("margin-left", -i);
            c.scrollFooterBox.css("margin-left", -i);
            ...
            if (c.cfg.virtualScroll) {
                ...
                clearTimeout(c.scrollTimeout);
                c.scrollTimeout = setTimeout(function () {
                    var l = c.scrollBody.outerHeight(), k = c.scrollBodyTable.outerHeight(), n = c.rowHeight * c.cfg.scrollStep, j = parseFloat((c.cfg.scrollLimit * c.rowHeight) + "px"), m = (j / n) || 1;
            ...
            if (c) {
                ...
                setTimeout(function () {
                    ...
                    if (f.hasColumnGroup) {
                        ...
                        k.find(IChartsFaces.escapeClientId(s.attr("id") + "_clone")).width(j);
                        v.eq(m).width(j)
                    }
                }, 1)
            } else {
                ...
                if (this.hasColumnGroup) {
                    ...
                    k.find(IChartsFaces.escapeClientId(s.attr("id") + "_clone")).width(j);
                    k.find(IChartsFaces.escapeClientId(g.attr("id") + "_clone")).width(o);
                    if (v.length > 0) {
...
IChartsFaces.widget.Dialog = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        ...
        if (this.cfg.appendTo) {
            this.parent = this.jq.parent();
            this.targetParent = IChartsFaces.expressions.SearchExpressionFacade.resolveComponentsAsSelector(this.cfg.appendTo);
        ...
        var a = this;
        this.jq.draggable({cancel: ".ui-dialog-content, .ui-dialog-titlebar-close", handle: ".ui-dialog-titlebar", containment: a.cfg.absolutePositioned ? "document" : "window", stop: function (d, e) {
...
IChartsFaces.widget.SelectOneMenu = IChartsFaces.widget.DeferredWidget.extend({init: function (a) {
        this._super(a);
        this.panelId = this.jqId + "_panel";
        this.input = $(this.jqId + "_input");
        ...
        var a = this.jq.attr("style"), b = a && a.indexOf("width") != -1;
...
IChartsFaces.widget.SelectBooleanCheckbox = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        ...
        if (!this.disabled) {
            this.box.on("mouseover.selectBooleanCheckbox", function () {
                ...
                if (IChartsFaces.env.browser.msie && IChartsFaces.env.isLtIE(9)) {
...
IChartsFaces.widget.SelectManyMenu = IChartsFaces.widget.SelectListbox.extend({init: function (a) {
        this._super(a);
...
IChartsFaces.widget.SelectCheckboxMenu = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        ...
        for (var d = 0; d < this.inputs.length; d++) {
            var m = this.inputs.eq(d), l = m.next(), c = m.is(":disabled"), o = m.is(":checked"), k = m.attr("title"), f = "ui-chkbox-box ui-widget ui-corner-all ui-state-default", j = "ui-selectcheckboxmenu-item ui-selectcheckboxmenu-list-item ui-corner-all", a = m.data("escaped");
            if (c) {
                f += " ui-state-disabled"
            }
            if (o) {
            ...
            var h = o ? "ui-chkbox-icon ui-icon ui-icon-check" : "ui-chkbox-icon ui-icon ui-icon-blank", j = o ? j + " ui-selectcheckboxmenu-checked" : j + " ui-selectcheckboxmenu-unchecked";
            var p = $('<li class="' + j + '"></li>');
            ...
            var b = $("<label></label>"), n = l.html().trim(), e = n.length;
            if (e > 0 && n !== "&nbsp;") {
                if (a) {
                    b.text(l.text())
                } else {
        ...
        if (!e.hasClass("ui-state-disabled")) {
            var a = e.prev().children("input"), c = e.closest("li.ui-selectcheckboxmenu-item");
            a.prop("checked", true).attr("aria-checked", true);
            if (d) {
        ...
        if (!e.hasClass("ui-state-disabled")) {
            var b = e.prev().children("input"), c = e.closest("li.ui-selectcheckboxmenu-item");
        ...
        var b = this.inputs.eq(g.index()), h = b.data("escaped"), a = b.next().html().trim(), f = a.length, e = f > 0 && a !== "&nbsp;" ? (h ? b.next().text() : b.next().html()) : b.val(), d = '<li class="ui-selectcheckboxmenu-token ui-state-active ui-corner-all" data-item-value="' + b.val() + '">';
...
IChartsFaces.widget.SlideMenu = IChartsFaces.widget.Menu.extend({init: function (b) {
        this._super(b);
        ...
        if (!this.jq.hasClass("ui-menu-dynamic")) {
            if (this.jq.is(":not(:visible)")) {
                var a = this.jq.closest(".ui-hidden-container"), c = this;
                if (a.length) {
                    IChartsFaces.addDeferredRender(this.id, a.attr("id"), function () {
                        return c.render()
                    })
                }
            } else {
...
IChartsFaces.widget.ContextMenu = IChartsFaces.widget.TieredMenu.extend({init: function (b) {
        ...
        this.jqTargetId = d ? document : IChartsFaces.escapeClientId(this.cfg.target);
        ...
        if (d) {
            ...
            if (this.cfg.targetWidgetVar) {
                var f = IChartsFaces.widgets[this.cfg.targetWidgetVar];
                if (f) {
                    ...
                    IChartsFaces.warn("ContextMenu targets a widget which is not available yet. Please place the contextMenu after the target component. targetWidgetVar: " + this.cfg.targetWidgetVar)
                }
            }
            if (c === false) {
        ...
        this.jq.css({left: d, top: c, "z-index": ++IChartsFaces.zindex}).show();
        h.preventDefault();
        h.stopPropagation()
    }, hide: function () {
        var a = this;
...
IChartsFaces.widget.MegaMenu = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        ...
        if (this.cfg.autoDisplay === false) {
            ...
            this.rootLinks.click(function (g) {
                var d = $(this), f = d.parent(), c = d.next();
                if (c.length === 1) {
                    if (c.is(":visible")) {
                        a.active = false;
                        a.deactivate(f, true)
...
IChartsFaces.widget.PanelMenu = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        ...
        this.menuContent.off("keydown.panelmenu blur.panelmenu").on("keydown.panelmenu", function (k) {
            ...
            if (IChartsFaces.env.isIE() && !b.focusCheck) {
                return
            }
            b.removeFocusedItem()
        });
...
IChartsFaces.widget.OverlayPanel = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
...
IChartsFaces.widget.Paginator = IChartsFaces.widget.BaseWidget.extend({init: function (b) {
        this.cfg = b;
        this.jq = $();
        var a = this;
        ...
        this.cfg.ariaPageLabel = IChartsFaces.getAriaLabel("paginator.PAGE");
        this.bindEvents()
    }, bindEvents: function () {
        var a = this;
...
IChartsFaces.widget.PickList = IChartsFaces.widget.BaseWidget.extend({init: function (c) {
        this._super(c);
        ...
        if (this.cfg.disabled) {
            ...
            var e = this, d = true;
            $(this.jqId + " ul").sortable({cancel: ".ui-state-disabled,.ui-chkbox-box", connectWith: this.jqId + " .ui-picklist-list", revert: 1, update: function (g, h) {
        ...
        this.jq.off("focus.pickList blur.pickList keydown.pickList", a).on("focus.pickList", a, null, function (d) {
            ...
            IChartsFaces.scrollInView(c, b.focusedItem);
                ...
                case h.UP:
                    ...
                    if (!b.focusedItem.hasClass("ui-state-highlight")) {
                        ...
                        if (d.length) {
                            ...
                            b.selectItem(d);
                            b.focusedItem = d;
                            IChartsFaces.scrollInView(g, b.focusedItem)
                        }
                    }
                    b.ariaRegion.text(b.focusedItem.data("item-label"));
                    i.preventDefault();
        ...
        b.children(".ui-picklist-item").each(function () {
            var f = $(this), g = IChartsFaces.escapeHTML(f.attr("data-item-value")), e = f.attr("data-item-label"), d = (e) ? IChartsFaces.escapeHTML(e) : "", c = $('<option selected="selected"></option>');
        ...
        if (this.hasBehavior("select")) {
            var d = this.cfg.behaviors.select, a = this.getListName(c), e = (a === "source") ? this.sourceInput : this.targetInput, b = {params: [{name: this.id + "_itemIndex", value: c.index()}, {name: this.id + "_listName", value: a}], onstart: function () {
...
IChartsFaces.widget.TabView = IChartsFaces.widget.DeferredWidget.extend({init: function (a) {
        ...
        a.on("focus.tabview", function (d) {
            ...
            var f = $.ui.keyCode, d = g.which, c = $(this);
...
IChartsFaces.widget.Tooltip = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        ...
        this.jq.css({left: a.left, top: a.top})
    }, align: function () {
        var c = this;
        this.jq.css({left: "", top: "", "z-index": ++IChartsFaces.zindex});
        if (this.cfg.trackMouse && this.mouseEvent) {
            this.jq.position({my: "left top+15", at: "right bottom", of: this.mouseEvent, collision: "flipfit", using: function (e, d) {
            ...
            this.jq.position({my: a, at: b, of: this.target, collision: "flipfit", using: function (e, d) {
                    c.alignUsing.call(c, e, d)
                }})
        }
    }, show: function () {
...
IChartsFaces.widget.VerticalTree = IChartsFaces.widget.BaseTree.extend({init: function (a) {
        ...
        this.jq.off("keydown.tree blur.tree", ".ui-treenode-label").on("keydown.tree", ".ui-treenode-label", null, function (k) {
            ...
            switch (k.which) {
                case n.LEFT:
                    var f = a.focusedNode.data("rowkey").toString(), o = f.length;
                ...
                case n.RIGHT:
                    ...
                    break;
                case n.UP:
                    var g = null, c = a.focusedNode.prev();
                    if (c.length) {
                    ...
                    break;
                case n.DOWN:
                    var g = null, d = a.focusedNode.find("> ul > li:visible:first");
                    if (d.length) {
                        g = d
                    } else {
        ...
        var b = this, a = {source: this.id, update: this.id, process: this.id, global: false, formId: this.cfg.formId, params: [{name: this.id + "_filtering", value: true}, {name: this.id + "_encodeFeature", value: true}], onsuccess: function (e, c, d) {
                IChartsFaces.ajax.Response.handle(e, c, d, {widget: b, handle: function (f) {
                        b.container.html(f)
                    }});
                return true
...
IChartsFaces.widget.TreeTable = IChartsFaces.widget.DeferredWidget.extend({init: function (a) {
        ...
        var c = this, b = {source: this.id, update: this.id, process: this.id, params: [{name: this.id + "_pagination", value: true}, {name: this.id + "_first", value: d.first}, {name: this.id + "_rows", value: d.rows}], onsuccess: function (g, e, f) {
                IChartsFaces.ajax.Response.handle(g, e, f, {widget: c, handle: function (h) {
                        this.tbody.html(h)
                    }});
                return true
        ...
        var b = {source: this.id, process: this.id, update: this.id, params: [{name: this.id + "_cellInfo", value: c}, {name: f, value: f}], onsuccess: function (j, h, i) {
                IChartsFaces.ajax.Response.handle(j, h, i, {widget: g, handle: function (k) {
...
IChartsFaces.widget.TriStateCheckbox = IChartsFaces.widget.BaseWidget.extend({init: function (b) {
        this._super(b);
        this.input = $(this.jqId + "_input");
        this.box = this.jq.find(".ui-chkbox-box");
        this.icon = this.box.children(".ui-chkbox-icon");
...
IChartsFaces.widget.Chips = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        this.input = $(this.jqId + "_input");
        this.hinput = $(this.jqId + "_hinput");
            ...
            switch (g.which) {
                case 8:
                    if (f.length === 0 && b.hinput.children("option") && b.hinput.children("option").length > 0) {
                        var c = b.hinput.children("option:last"), d = c.index();
        ...
        var b = IChartsFaces.escapeHTML(c);
        ...
        if (this.cfg.behaviors) {
            ...
            if (b) {
                var a = {params: [{name: this.id + "_itemSelect", value: c}]};
                b.call(this, a)
            }
        }
        ...
        if (this.cfg.behaviors) {
            var a = this.cfg.behaviors.itemUnselect;
            if (a) {
                var b = {params: [{name: this.id + "_itemUnselect", value: c}]};
                a.call(this, b)
            }
        }
    }});
