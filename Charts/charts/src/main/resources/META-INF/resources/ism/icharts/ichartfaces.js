...
IChartsFaces.env = {mobile: false, touch: false, ios: false, browser: null, init: function () {
...
IChartsFaces.AB_MAPPING = {s: "source", f: "formId", p: "process", u: "update", e: "event", a: "async", g: "global", d: "delay", t: "timeout", iau: "ignoreAutoUpdate", ps: "partialSubmit", psf: "partialSubmitFilter", rv: "resetValues", fi: "fragmentId", fu: "fragmentUpdate", pa: "params", onst: "onstart", oner: "onerror", onsu: "onsuccess", onco: "oncomplete"};
...
IChartsFaces.ajax = {VIEW_HEAD: "javax.faces.ViewHead", VIEW_BODY: "javax.faces.ViewBody", Utils: {getContent: function (c) {
            var b = "";
        ...
        updateElement: function (f, c, e) {
            if (f.indexOf(IChartsFaces.VIEW_STATE) !== -1) {
                IChartsFaces.ajax.Utils.updateFormStateInput(IChartsFaces.VIEW_STATE, c, e)
            } else {
                if (f.indexOf(IChartsFaces.CLIENT_WINDOW) !== -1) {
                    IChartsFaces.ajax.Utils.updateFormStateInput(IChartsFaces.CLIENT_WINDOW, c, e)
                } else {
                    if (f === IChartsFaces.VIEW_ROOT) {
                        window.IChartsFaces = null;
                        var b = $.ajaxSetup()["cache"];
                        $.ajaxSetup()["cache"] = true;
                        $("head").html(c.substring(c.indexOf("<head>") + 6, c.lastIndexOf("</head>")));
                        $.ajaxSetup()["cache"] = b;
                        var d = new RegExp("<body[^>]*>", "gi").exec(c)[0];
                        var a = c.indexOf(d) + d.length;
                        $("body").html(c.substring(a, c.lastIndexOf("</body>")))
                    } else {
                        if (f === IChartsFaces.ajax.VIEW_HEAD) {
                            window.IChartsFaces = null;
                            var b = $.ajaxSetup()["cache"];
                            $.ajaxSetup()["cache"] = true;
                            $("head").html(c.substring(c.indexOf("<head>") + 6, c.lastIndexOf("</head>")));
                            ...
                            if (f === IChartsFaces.ajax.VIEW_BODY) {
                                var d = new RegExp("<body[^>]*>", "gi").exec(c)[0];
                                var a = c.indexOf(d) + d.length;
                                $("body").html(c.substring(a, c.lastIndexOf("</body>")))
                            } else {
                                $(IChartsFaces.escapeClientId(f)).replaceWith(c)
                            }
                        }
                    }
                }
            ...
            var n = (e.global === true || e.global === undefined) ? true : false, b = null, f = null;
            if (e.onstart) {
            ...
            if (e.formId) {
                ...
                b = $(IChartsFaces.escapeClientId(f)).closest("form");
                if (b.length === 0) {
                    b = $("form").eq(0)
                }
            }
            IChartsFaces.debug("Form to post " + b.attr("id") + ".");
            var u = b.attr("action"), s = b.children("input[name*='javax.faces.encodedURL']"), g = [];
            var m = null;
            if (s.length > 0) {
                m = 'form[action="' + u + '"]';
                u = s.val()
            }
            IChartsFaces.debug("URL to post " + u + ".");
            var l = IChartsFaces.ajax.Request.extractParameterNamespace(b);
            IChartsFaces.ajax.Request.addParam(g, IChartsFaces.PARTIAL_REQUEST_PARAM, true, l);
            IChartsFaces.ajax.Request.addParam(g, IChartsFaces.PARTIAL_SOURCE_PARAM, f, l);
            if (e.resetValues) {
                IChartsFaces.ajax.Request.addParam(g, IChartsFaces.RESET_VALUES_PARAM, true, l)
            }
            if (e.ignoreAutoUpdate) {
                IChartsFaces.ajax.Request.addParam(g, IChartsFaces.IGNORE_AUTO_UPDATE_PARAM, true, l)
            }
            ...
            if (a !== "@none") {
                IChartsFaces.ajax.Request.addParam(g, IChartsFaces.PARTIAL_PROCESS_PARAM, a, l)
            }
            ...
            if (d.length > 0) {
                IChartsFaces.ajax.Request.addParam(g, IChartsFaces.PARTIAL_UPDATE_PARAM, d.join(" "), l)
            }
            if (e.event) {
                IChartsFaces.ajax.Request.addParam(g, IChartsFaces.BEHAVIOR_EVENT_PARAM, e.event, l);
                var k = e.event;
                ...
                IChartsFaces.ajax.Request.addParam(g, IChartsFaces.PARTIAL_EVENT_PARAM, k, l)
            } else {
                IChartsFaces.ajax.Request.addParam(g, f, f, l)
            }
            if (e.params) {
                IChartsFaces.ajax.Request.addParams(g, e.params, l)
            }
            if (e.ext && e.ext.params) {
                IChartsFaces.ajax.Request.addParams(g, e.ext.params, l)
            }
            if (e.partialSubmit && a.indexOf("@all") === -1) {
                var o = false, h = e.partialSubmitFilter || ":input";
                if (a.indexOf("@none") === -1) {
                    for (var p = 0; p < r.length; p++) {
                        var j = $(IChartsFaces.escapeClientId(r[p]));
                        var v = null;
                        if (j.is("form")) {
                            v = j.serializeArray();
                            o = true
                ...
                if (!o) {
                    IChartsFaces.ajax.Request.addParamFromInput(g, IChartsFaces.VIEW_STATE, b, l);
                    IChartsFaces.ajax.Request.addParamFromInput(g, IChartsFaces.CLIENT_WINDOW, b, l);
                    IChartsFaces.ajax.Request.addParamFromInput(g, "dsPostWindowId", b, l);
                    IChartsFaces.ajax.Request.addParamFromInput(g, "dspwid", b, l)
                }
            } else {
            ...
            var q = {url: u, type: "POST", cache: false, dataType: "xml", data: c, portletForms: m, source: e.source, global: false, beforeSend: function (w, i) {
                    ...
                    if (w) {
                        ...
                        IChartsFaces.ajax.Response.handle(x, i, y)
                    }
                    ...
                    IChartsFaces.ajax.Queue.removeXHR(w);
                    if (!e.async) {
                        IChartsFaces.ajax.Queue.poll()
                    }
                }};
            ...
            IChartsFaces.ajax.Queue.addXHR($.ajax(q))
            ...
            if (a && a.length > 0) {
                var d = a.val();
                IChartsFaces.ajax.Request.addParam(f, b, d, e)
            }
            ...
            return null
        }}, Response: {handle: function (f, d, k, b) {
                ...
                switch (a.nodeName) {
                    ...
                    case"changes":
                        ...
                        for (var c = 0; c < a.childNodes.length; c++) {
                            ...
                            switch (h.nodeName) {
                                case"update":
                                    IChartsFaces.ajax.ResponseProcessor.doUpdate(h, k, b);
                                    break;
                                case"delete":
                                    IChartsFaces.ajax.ResponseProcessor.doDelete(h);
                                    break;
                                case"insert":
                                    IChartsFaces.ajax.ResponseProcessor.doInsert(h);
                                    break;
                                case"attributes":
                                    IChartsFaces.ajax.ResponseProcessor.doAttributes(h);
                                    break;
                                case"eval":
                                    IChartsFaces.ajax.ResponseProcessor.doEval(h);
                                    break;
                                case"extension":
                                    IChartsFaces.ajax.ResponseProcessor.doExtension(h, k);
                                    break
                            }
                        }
                        IChartsFaces.ajax.Response.handleReFocus(g);
                    ...
                    case"eval":
                        ...
                        break;
                    case"extension":
                        IChartsFaces.ajax.ResponseProcessor.doExtension(a, k);
                        break;
                    case"error":
                        IChartsFaces.ajax.ResponseProcessor.doError(a, k);
                        break
                }
            }
        }, handleReFocus: function (b) {
            if (IChartsFaces.customFocus === false && b && b !== $(document.activeElement).attr("id")) {
                var a = $(IChartsFaces.escapeClientId(b));
                a.focus();
                setTimeout(function () {
                    if (!a.is(":focus")) {
                        a.focus()
        ...
        }}, ResponseProcessor: {doRedirect: function (a) {
...
IChartsFaces.dialog.DialogHandler = {openDialog: function (e) {
        ...
        var c = d.find("iframe"), b = e.url.indexOf("?") === -1 ? "?" : "&", a = e.url + b + "pfdlgcid=" + e.pfdlgcid, g = e.options.contentWidth || 640;
        c.width(g);
        c.on("load", function () {
            var o = $(this), k = o.contents().find("title"), n = false;
            if (e.options.headerElement) {
                var m = IChartsFaces.escapeClientId(e.options.headerElement), j = c.contents().find(m);
                if (j.length) {
                    k = j;
                    n = true
                }
            ...
            if (!o.data("initialized")) {
                IChartsFaces.cw("DynamicDialog", h, {id: i, position: "center", sourceComponentId: e.sourceComponentId, sourceWidget: e.sourceWidget, onHide: function () {
                        var r = this, q = this.content.children("iframe");
                        if (q.get(0).contentWindow.IChartsFaces) {
                            this.destroyIntervalId = setInterval(function () {
                                if (q.get(0).contentWindow.IChartsFaces.ajax.Queue.isEmpty()) {
                                    clearInterval(r.destroyIntervalId);
                                    q.attr("src", "about:blank");
                                    r.jq.remove()
                                }
                            }, 10)
                        } else {
            ...
            var p = PF(h).titlebar.children("span.ui-dialog-title");
            if (k.length > 0) {
                if (n) {
                    p.append(k);
                    k.show()
            ...
            if (e.options.contentHeight) {
                ...
                l = o.get(0).contentWindow.document.body.scrollHeight + (IChartsFaces.env.browser.webkit ? 5 : 20)
            }
            o.css("height", l);
        ...
        if (!this.messageDialog) {
            ...
            IChartsFaces.cw("Dialog", "primefacesmessagedialog", {id: "primefacesmessagedlg", modal: true, draggable: false, resizable: false, showEffect: "fade", hideEffect: "fade"});
        ...
        if (IChartsFaces.confirmDialog) {
            IChartsFaces.confirmSource = (typeof (a.source) === "string") ? $(IChartsFaces.escapeClientId(a.source)) : $(a.source);
            IChartsFaces.confirmDialog.showMessage(a)
        } else {
            IChartsFaces.warn("No global confirmation dialog available.")
        }
    }};
IChartsFaces.widget.AccordionPanel = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        ...
        for (var c = 0; c < this.headers.length; c++) {
            if (b === c && !this.headers.eq(c).hasClass("ui-state-disabled")) {
        ...
        this.headers.on("focus.accordion", function () {
            ...
            if (!a.focusedHeader.hasClass("ui-state-disabled")) {
                a.headers.filter('[tabindex="0"]').attr("tabindex", "-1").removeClass("ui-tabs-outline");
                a.focusedHeader.attr("tabindex", "0").addClass("ui-tabs-outline")
            }
                ...
                case b.UP:
                    if (a.focusedHeader) {
                        if ((a.focusedHeader.index() / 2) === 0) {
                            a.focusedHeader = a.headers.filter("h3:not(.ui-state-disabled):last")
                        } else {
                            a.focusedHeader = a.focusedHeader.prevAll("h3:not(.ui-state-disabled):first");
                            if (!a.focusedHeader.length) {
                                a.focusedHeader = a.headers.filter("h3:not(.ui-state-disabled):last")
                            }
                        }
                        a.focusedHeader.trigger("focus.accordion")
                    }
                    c.preventDefault();
                    break;
                case b.RIGHT:
                case b.DOWN:
                    if (a.focusedHeader) {
                        if ((a.focusedHeader.index() / 2) === (a.headers.length - 1)) {
                            a.focusedHeader = a.headers.filter("h3:not(.ui-state-disabled):first")
                        } else {
                            a.focusedHeader = a.focusedHeader.nextAll("h3:not(.ui-state-disabled):first");
                            if (!a.focusedHeader.length) {
                                a.focusedHeader = a.headers.filter("h3:not(.ui-state-disabled):first")
                            }
                        }
    ...
    loadDynamicTab: function (a) {
        var c = this, b = {source: this.id, process: this.id, update: this.id, params: [{name: this.id + "_contentLoad", value: true}, {name: this.id + "_newTab", value: a.attr("id")}, {name: this.id + "_tabindex", value: parseInt(a.index() / 2)}], onsuccess: function (g, e, f) {
                IChartsFaces.ajax.Response.handle(g, e, f, {widget: c, handle: function (h) {
                        a.html(h);
                        if (this.cfg.cache) {
                            this.markAsLoaded(a)
                        }
        ...
        if (this.hasBehavior("tabChange")) {
            ...
            IChartsFaces.ajax.AjaxRequest(b)
        }
    }, fireTabChangeEvent: function (a) {
        var c = this.cfg.behaviors.tabChange, b = {params: [{name: this.id + "_newTab", value: a.attr("id")}, {name: this.id + "_tabindex", value: parseInt(a.index() / 2)}]};
        c.call(this, b)
    }, fireTabCloseEvent: function (a) {
        var c = this.cfg.behaviors.tabClose, b = {params: [{name: this.id + "_tabId", value: a.attr("id")}, {name: this.id + "_tabindex", value: parseInt(a.index() / 2)}]};
        c.call(this, b)
...
IChartsFaces.widget.AutoComplete = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        ...
        this.input.data(IChartsFaces.CLIENT_ID_DATA, this.id);
        this.hinput.data(IChartsFaces.CLIENT_ID_DATA, this.id);
        if (this.cfg.multiple) {
            ...
            IChartsFaces.skinInput(this.input);
        ...
        if (this.cfg.behaviors) {
            IChartsFaces.attachBehaviors(this.input, this.cfg.behaviors)
        }
        ...
        var a = this.cfg.appendTo ? IChartsFaces.expressions.SearchExpressionFacade.resolveComponentsAsSelector(this.cfg.appendTo) : $(document.body);
        if (!a.is(this.jq)) {
        ...
        if (IChartsFaces.env.browser.mobile) {
        ...
        if (this.cfg.queryEvent !== "enter") {
            this.input.on("input propertychange", function (d) {
                ...
                if (IChartsFaces.isIE(8) && (a.itemClick || d.originalEvent.propertyName !== "value")) {
                    a.itemClick = false;
                    return
                }
                var c = a.input.val();
                ...
                switch (g.which) {
                    case f.UP:
                        ...
                        if (c.length == 1) {
                            ...
                            if (a.cfg.scrollHeight) {
                                IChartsFaces.scrollInView(a.panel, c)
                            }
                            if (a.cfg.itemtip) {
                                a.showItemtip(c)
                            }
                    ...
                    case f.DOWN:
                        ...
                        if (b.length == 1) {
                            ...
                            if (a.cfg.scrollHeight) {
                                IChartsFaces.scrollInView(a.panel, b)
                            }
                            if (a.cfg.itemtip) {
                                a.showItemtip(b)
                            }
        ...
        this.items.bind("mouseover", function () {
            ...
            var d = $(this), f = d.attr("data-item-value");
            if (IChartsFaces.isIE(8)) {
                a.itemClick = true
            }
            if (a.cfg.multiple) {
                var b = '<li data-token-value="' + d.attr("data-item-value") + '"class="ui-autocomplete-token ui-state-active ui-corner-all ui-helper-hidden">';
                b += '<span class="ui-autocomplete-token-icon ui-icon ui-icon-close" />';
                b += '<span class="ui-autocomplete-token-label">' + d.attr("data-item-label") + "</span></li>";
                a.inputContainer.before(b);
                a.multiItemContainer.children(".ui-helper-hidden").fadeIn();
                a.input.val("").focus();
                a.hinput.append('<option value="' + f + '" selected="selected"></option>');
                if (a.multiItemContainer.children("li.ui-autocomplete-token").length >= a.cfg.selectLimit) {
                    a.input.css("display", "none").blur()
                }
            } else {
                a.input.val(d.attr("data-item-label")).focus();
                this.currentText = a.input.val();
                this.previousText = a.input.val();
                if (a.cfg.pojo) {
                    a.hinput.val(f)
                }
                if (IChartsFaces.env.isLtIE(10)) {
        ...
        this.itemtip.html(b.html()).css({left: "", top: "", "z-index": ++IChartsFaces.zindex, width: b.outerWidth()}).position({my: this.cfg.itemtipMyPosition, at: this.cfg.itemtipAtPosition, of: c});
        ...
        if (this.items.length > 0) {
            ...
            if (this.panel.children().is("ul") && c.length > 0) {
                this.items.each(function () {
                    var g = $(this), i = g.html(), f = new RegExp(IChartsFaces.escapeRegExp(c), "gi"), h = i.replace(f, '<span class="ui-autocomplete-query">$&</span>');
                    g.html(h)
                })
            }
            if (this.cfg.forceSelection) {
        ...
        var b = {source: this.id, process: this.id, update: this.id, formId: this.cfg.formId, onsuccess: function (g, e, f) {
                IChartsFaces.ajax.Response.handle(g, e, f, {widget: d, handle: function (h) {
                        this.panel.html(h);
                        if (this.cfg.cache) {
                            this.cache[c] = h
                        }
        ...
        this.panel.css({left: "", top: "", width: c, "z-index": ++IChartsFaces.zindex});
        ...
        if (this.items.length) {
            ...
            this.items.eq(0).before(this.getGroupItem(a.currentGroup, a.itemContainer, b));
            this.items.each(function (d) {
                var e = a.items.eq(d), f = e.data("item-group"), c = e.data("item-group-tooltip");
                if (a.currentGroup !== f) {
                    a.currentGroup = f;
...
IChartsFaces.widget.BlockUI = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this.cfg = a;
        this.id = this.cfg.id;
        this.jqId = IChartsFaces.escapeClientId(this.id);
        this.block = IChartsFaces.expressions.SearchExpressionFacade.resolveComponentsAsSelector(this.cfg.block);
        ...
        var b = this, a = IChartsFaces.expressions.SearchExpressionFacade.resolveComponents(this.cfg.triggers);
        ...
        this.blocker.css("z-index", ++IChartsFaces.zindex);
        this.content.css({left: (this.blocker.width() - this.content.outerWidth()) / 2, top: (this.blocker.height() - this.content.outerHeight()) / 2, "z-index": ++IChartsFaces.zindex});
        if (this.cfg.animate) {
            this.blocker.fadeIn()
        } else {
            this.blocker.show()
...
IChartsFaces.widget.Calendar = IChartsFaces.widget.BaseWidget.extend({init: function (b) {
        this._super(b);
        this.input = $(this.jqId + "_input");
        ...
        if (this.cfg.popup) {
            IChartsFaces.skinInput(this.jqEl);
            if (this.cfg.behaviors) {
                IChartsFaces.attachBehaviors(this.jqEl, this.cfg.behaviors)
            }
            this.cfg.beforeShow = function (g, i) {
                setTimeout(function () {
                    $("#ui-datepicker-div").css("z-index", ++IChartsFaces.zindex)
                }, 1);
                if (IChartsFaces.env.touch && !a.input.attr("readonly") && a.cfg.showOn && a.cfg.showOn === "button") {
        ...
        if (IChartsFaces.env.touch && !this.input.attr("readonly") && this.cfg.showOn && this.cfg.showOn === "button") {
        ...
        this.input.data(IChartsFaces.CLIENT_ID_DATA, this.id);
        if (this.cfg.mask) {
            var c = {placeholder: this.cfg.maskSlotChar || "_"};
        ...
        var a = IChartsFaces.locales[this.cfg.locale];
        if (a) {
            for (var b in a) {
                this.cfg[b] = a[b]
            }
...
IChartsFaces.widget.Carousel = IChartsFaces.widget.DeferredWidget.extend({init: function (a) {
        this._super(a);
...
IChartsFaces.widget.ColumnToggler = IChartsFaces.widget.DeferredWidget.extend({init: function (a) {
        this._super(a);
        this.table = IChartsFaces.expressions.SearchExpressionFacade.resolveComponentsAsSelector(this.cfg.datasource);
        this.trigger = IChartsFaces.expressions.SearchExpressionFacade.resolveComponentsAsSelector(this.cfg.trigger);
        this.tableId = this.table.attr("id");
        this.thead = $(IChartsFaces.escapeClientId(this.tableId) + "_head");
        this.tbody = $(IChartsFaces.escapeClientId(this.tableId) + "_data");
        this.tfoot = $(IChartsFaces.escapeClientId(this.tableId) + "_foot");
        this.visible = false;
        this.render();
        this.bindEvents()
    }, render: function () {
        ...
        for (var b = 0; b < this.columns.length; b++) {
            var c = this.columns.eq(b), d = c.hasClass("ui-helper-hidden"), e = d ? "ui-chkbox-box ui-widget ui-corner-all ui-state-default" : "ui-chkbox-box ui-widget ui-corner-all ui-state-default ui-state-active", a = (d) ? "ui-chkbox-icon ui-icon ui-icon-blank" : "ui-chkbox-icon ui-icon ui-icon-check";
            $('<li class="ui-columntoggler-item"><div class="ui-chkbox ui-widget"><div class="' + e + '"><span class="' + a + '"></span></div></div><label>' + c.children(".ui-column-title").text() + "</label></li>").data("column", c.attr("id")).appendTo(this.itemContainer)
        }
        ...
        b.addClass("ui-state-active").removeClass("ui-state-hover").children(".ui-chkbox-icon").addClass("ui-icon-check").removeClass("ui-icon-blank");
        var a = $(document.getElementById(b.closest("li.ui-columntoggler-item").data("column"))).index() + 1, c = this.thead.children("tr").find("th:nth-child(" + a + ")");
        c.removeClass("ui-helper-hidden");
        $(IChartsFaces.escapeClientId(c.attr("id") + "_clone")).removeClass("ui-helper-hidden");
        this.tbody.children("tr").find("td:nth-child(" + a + ")").removeClass("ui-helper-hidden");
        this.tfoot.children("tr").find("td:nth-child(" + a + ")").removeClass("ui-helper-hidden");
        this.fireToggleEvent(true, (a - 1))
    }, uncheck: function (b) {
        b.removeClass("ui-state-active").children(".ui-chkbox-icon").addClass("ui-icon-blank").removeClass("ui-icon-check");
        var a = $(document.getElementById(b.closest("li.ui-columntoggler-item").data("column"))).index() + 1, c = this.thead.children("tr").find("th:nth-child(" + a + ")");
        c.addClass("ui-helper-hidden");
        $(IChartsFaces.escapeClientId(c.attr("id") + "_clone")).addClass("ui-helper-hidden");
        this.tbody.children("tr").find("td:nth-child(" + a + ")").addClass("ui-helper-hidden");
        this.tfoot.children("tr").find("td:nth-child(" + a + ")").addClass("ui-helper-hidden");
        this.fireToggleEvent(false, (a - 1))
    }, alignPanel: function () {
        this.panel.css({left: "", top: "", "z-index": ++IChartsFaces.zindex}).position({my: "left top", at: "left bottom", of: this.trigger});
...
IChartsFaces.widget.Dashboard = IChartsFaces.widget.BaseWidget.extend({init: function (b) {
        this._super(b);
...
IChartsFaces.widget.DataGrid = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        ...
        this.paginator = new IChartsFaces.widget.Paginator(this.cfg.paginator)
    }, hasBehavior: function (a) {
        if (this.cfg.behaviors) {
            return this.cfg.behaviors[a] !== undefined
        }
        ...
        if (this.hasBehavior("page")) {
            ...
            IChartsFaces.ajax.Request.handle(b)
        }
    }, getPaginator: function () {
        return this.paginator
    }});
IChartsFaces.widget.DataList = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
...
IChartsFaces.widget.DataScroller = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        ...
        var b = this, a = {source: this.id, process: this.id, update: this.id, global: false, params: [{name: this.id + "_load", value: true}, {name: this.id + "_offset", value: this.cfg.offset}], onsuccess: function (e, c, d) {
                IChartsFaces.ajax.Response.handle(e, c, d, {widget: b, handle: function (f) {
                        this.list.append(f)
                    }});
                return true
            }, oncomplete: function () {
        ...
        IChartsFaces.ajax.AjaxRequest(a)
    }, shouldLoad: function () {
        return(!this.loading && !this.allLoaded)
    }});
/**
 * IChartsFaces DataTable Widget
 */
IChartsFaces.widget.DataTable = IChartsFaces.widget.DeferredWidget.extend({
    SORT_ORDER: {ASCENDING: 1, DESCENDING: -1, UNSORTED: 0},
    init: function (cfg) {
        this._super(cfg);
    ...
    bindSortEvents: function () {
        ...
        // SORTING ASCENDING
        this.sortFilterAscending = $('li.ui-sort-filter-ascending');
        this.sortFilterAscending.attr('tabindex', this.cfg.tabindex);
        this.sortFilterAscending
                ...
                .on('click.dataTable', function (e, metaKeyOn) {
                    $this.sortFilterWay($this, this, "1", e, metaKeyOn);
                });
        // SORTING DESCENDING
        this.sortFilterDescending = $('li.ui-sort-filter-descending');
        this.sortFilterDescending.attr('tabindex', this.cfg.tabindex);
        this.sortFilterDescending
                .on('mouseenter.dataTable', function () {
                    var column = $(this);
                    if (!column.hasClass('ui-state-active'))
                        column.addClass('ui-state-hover');
        ...
        // SORTING UNSORTED
        this.sortFilterDescending = $('li.ui-sort-filter-unsorted');
        this.sortFilterDescending.attr('tabindex', this.cfg.tabindex);
        this.sortFilterDescending
                .on('mouseenter.dataTable', function () {
                    var column = $(this);
                ...
                .on('click.dataTable', function (e, metaKeyOn) {
                    $this.sortFilterWay($this, this, "0", e, metaKeyOn);
                });
        ...
        this.sortableColumns
                ...
                .on('click.dataTable', function (e, metaKeyOn) {
                    /*
                     ...
                     console.log("click sort columns");
                     IChartsFaces.clearSelection();
    ...
    sortFilterWay: function (mThis, cThis, sortWay, e, metaKeyOn) {
        $this = mThis;

        if (!$this.shouldSort(e, cThis)) {
            return;
        }
        IChartsFaces.clearSelection();

        var dataCh = "#" + $(cThis).parents(".ui-column-overlay").attr("data-ch");
        dataCh = dataCh.replace(/:/g, "\\:");
        var columnHeader = $(dataCh);//$(cThis).parents(".ui-sortable-column");
        var sortOrderData = columnHeader.data('sortorder') === undefined ? $this.SORT_ORDER.UNSORTED : columnHeader.data('sortorder');

        //sortOrderData = 1; // for ascending
        //var sortOrder = (sortOrderData === $this.SORT_ORDER.UNSORTED) ? $this.SORT_ORDER.ASCENDING : -1 * sortOrderData;
        var sortOrder = sortWay;
    ...
    setupFiltering: function () {
        ...
        filterColumns.children('.ui-column-filter').each(function () {
            ...
            if (filter.is('input:text')) {
                IChartsFaces.skinInput(filter);
                $this.bindTextFilter(filter);
            } else {
                IChartsFaces.skinSelect(filter);
    ...
    bindRowEvents: function () {
        ...
        this.tbody.off('click.dataTable', this.rowSelector)
    ...
    cloneHead: function () {
        ...
        if (this.cfg.scrollWidth) {
            ...
            clonedSortableColumns.on('blur.dataTable', function () {
                $(IChartsFaces.escapeClientId($(this).data('original'))).removeClass('ui-state-focus');
            })
                    .on('focus.dataTable', function () {
                        $(IChartsFaces.escapeClientId($(this).data('original'))).addClass('ui-state-focus');
                    })
                    .on('keydown.dataTable', function (e) {
                        var key = e.which,
                                keyCode = $.ui.keyCode;
                        if ((key === keyCode.ENTER || key === keyCode.NUMPAD_ENTER) && $(e.target).is(':not(:input)')) {
                            $(IChartsFaces.escapeClientId($(this).data('original'))).trigger('click.dataTable', (e.metaKey || e.ctrlKey));
                            e.preventDefault();
                        }
                    });
        }
    ...
    getScrollbarWidth: function () {
        if (!this.scrollbarWidth) {
            this.scrollbarWidth = IChartsFaces.env.browser.webkit ? '15' : IChartsFaces.calculateScrollbarWidth();
        }
    ...
    loadLiveRows: function () {
        ...
        var $this = this,
                options = {
                    ...
                    onsuccess: function (responseXML, status, xhr) {
                        IChartsFaces.ajax.Response.handle(responseXML, status, xhr, {
                            widget: $this,
        ...
        IChartsFaces.ajax.Request.handle(options);
    },
    /**
     * Ajax pagination
     */
    ...
    filter: function () {
        ...
        if (this.hasBehavior('filter')) {
            ...
            IChartsFaces.ajax.AjaxRequest(options);
        }
    },
                ...
                if (expandMode) {
                    ...
                    setTimeout(function () {
                        if ($this.hasColumnGroup) {
                            ...
                            $this.theadClone.find(IChartsFaces.escapeClientId(columnHeader.attr('id') + '_clone')).width(newWidth); //body
                    ...
                    if (this.hasColumnGroup) {
                        ...
                        //body
                        this.theadClone.find(IChartsFaces.escapeClientId(columnHeader.attr('id') + '_clone')).width(newWidth);
                        this.theadClone.find(IChartsFaces.escapeClientId(nextColumnHeader.attr('id') + '_clone')).width(nextColumnWidth);
                        //footer
                        if (this.footerCols.length > 0) {
                            var footerCol = this.footerCols.eq(colIndex),
    ...
    isSelected: function (rowIndex) {
        return IChartsFaces.inArray(this.selection, rowIndex);
    },
    ...
    makeRowsDraggable: function () {
        ...
        this.tbody.sortable({
            ...
            change: function (event, ui) {
                if ($this.cfg.scrollable) {
                    IChartsFaces.scrollInView($this.scrollBody, ui.placeholder);
                }
            }
        });
    },
...
/**
 * IChartsFaces DataTable with Frozen Columns Widget
 */
IChartsFaces.widget.FrozenDataTable = IChartsFaces.widget.DataTable.extend({
    setupScrolling: function () {
            ...
            if (expandMode) {
                ...
                setTimeout(function () {
                    ...
                    if ($this.hasColumnGroup) {
                        ...
                        theadClone.find(IChartsFaces.escapeClientId(columnHeader.attr('id') + '_clone')).width(newWidth); //body
                ...
                if (this.hasColumnGroup) {
                    ...
                    theadClone.find(IChartsFaces.escapeClientId(columnHeader.attr('id') + '_clone')).width(newWidth);
                    theadClone.find(IChartsFaces.escapeClientId(nextColumnHeader.attr('id') + '_clone')).width(nextColumnWidth);
                    if (footerCols.length > 0) {
                        var footerCol = footerCols.eq(colIndex),
                                nextFooterCol = footerCol.next();
                        footerCol.width(newWidth);
...
IChartsFaces.widget.Dialog = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        ...
        if (this.cfg.appendTo) {
            this.jq.appendTo(IChartsFaces.expressions.SearchExpressionFacade.resolveComponentsAsSelector(this.cfg.appendTo))
        }
        ...
        if (a.appendTo) {
            ...
            if (b.length > 1) {
                IChartsFaces.expressions.SearchExpressionFacade.resolveComponentsAsSelector(a.appendTo).children(this.jqId).remove()
            }
        }
        this.init(a)
    }, initSize: function () {
        ...
        if (this.cfg.width === "auto" && IChartsFaces.isIE(7)) {
        ...
        if (this.cfg.focus) {
            IChartsFaces.expressions.SearchExpressionFacade.resolveComponentsAsSelector(this.cfg.focus).focus()
        } else {
        ...
        if (this.cfg.closeOnEscape) {
            $(document).on("keydown.dialog_" + this.id, function (d) {
                var c = $.ui.keyCode, b = parseInt(a.jq.css("z-index")) === IChartsFaces.zindex;
        ...
        this.jq.css("z-index", ++IChartsFaces.zindex)
...
IChartsFaces.widget.ConfirmDialog = IChartsFaces.widget.Dialog.extend({init: function (a) {
        a.draggable = false;
        a.resizable = false;
        a.modal = true;
        ...
        if (this.cfg.global) {
            IChartsFaces.confirmDialog = this;
            this.jq.find(".ui-confirmdialog-yes").on("click.ui-confirmdialog", function (c) {
                if (IChartsFaces.confirmSource) {
                    var b = new Function("event", IChartsFaces.confirmSource.data("pfconfirmcommand"));
                    b.call(IChartsFaces.confirmSource.get(0), c);
                    IChartsFaces.confirmDialog.hide();
                    IChartsFaces.confirmSource = null
                }
                c.preventDefault()
            });
...
IChartsFaces.widget.DynamicDialog = IChartsFaces.widget.Dialog.extend({show: function () {
...
IChartsFaces.widget.Draggable = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this.cfg = a;
        this.id = this.cfg.id;
        this.jqId = IChartsFaces.escapeClientId(this.id);
        this.jq = $(IChartsFaces.escapeClientId(this.cfg.target));
        if (this.cfg.appendTo) {
            this.cfg.appendTo = IChartsFaces.expressions.SearchExpressionFacade.resolveComponentsAsSelector(this.cfg.appendTo)
        }
        this.jq.draggable(this.cfg);
        this.removeScriptElement(this.id)
    }});
IChartsFaces.widget.Droppable = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this.cfg = a;
        this.id = this.cfg.id;
        this.jqId = IChartsFaces.escapeClientId(this.id);
        this.jq = $(IChartsFaces.escapeClientId(this.cfg.target));
...
IChartsFaces.widget.Effect = IChartsFaces.widget.BaseWidget.extend({init: function (b) {
        this.cfg = b;
        this.id = this.cfg.id;
        this.jqId = IChartsFaces.escapeClientId(this.id);
        this.source = $(IChartsFaces.escapeClientId(this.cfg.source));
        var a = this;
...
IChartsFaces.widget.Fieldset = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
...
IChartsFaces.widget.InputText = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        IChartsFaces.skinInput(this.jq)
    }, disable: function () {
        this.jq.prop("disabled", true).addClass("ui-state-disabled")
    }, enable: function () {
        this.jq.prop("disabled", false).removeClass("ui-state-disabled")
    }});
IChartsFaces.widget.InputTextarea = IChartsFaces.widget.DeferredWidget.extend({init: function (a) {
        this._super(a);
        ...
        if (this.cfg.counter) {
            this.counter = this.cfg.counter ? $(IChartsFaces.escapeClientId(this.cfg.counter)) : null;
        ...
        if (a.autoComplete) {
            $(IChartsFaces.escapeClientId(a.id + "_panel")).remove()
        }
        this.init(a)
                ...
                case i.LEFT:
                    if (d) {
                        ...
                        if (g.length == 1) {
                            ...
                            if (a.cfg.scrollHeight) {
                                IChartsFaces.scrollInView(a.panel, g)
                            }
                        }
                        j.preventDefault()
                    } else {
                ...
                case i.RIGHT:
                    if (d) {
                        ...
                        if (f.length == 1) {
                            ...
                            if (a.cfg.scrollHeight) {
                                IChartsFaces.scrollInView(a.panel, f)
                            }
                        }
                        j.preventDefault()
                    } else {
        ...
        var c = this, a = {source: this.id, update: this.id, process: this.id, params: [{name: this.id + "_query", value: b}], onsuccess: function (f, d, e) {
                IChartsFaces.ajax.Response.handle(f, d, e, {widget: c, handle: function (g) {
                        this.panel.html(g);
        ...
        this.panel.css({left: a.left + b.left, top: a.top + b.top, width: this.jq.innerWidth(), "z-index": ++IChartsFaces.zindex})
    }, show: function () {
        this.alignPanel();
        this.panel.show()
    }, hide: function () {
        ...
        if (IChartsFaces.env.browser.msie) {
            return(parseInt(IChartsFaces.env.browser.version, 10) > 9)
        } else {
            if (IChartsFaces.env.browser.opera) {
                return(parseInt(IChartsFaces.env.browser.version, 10) > 12)
            } else {
                return true
            }
        }
    }});
IChartsFaces.widget.SelectOneMenu = IChartsFaces.widget.BaseWidget.extend({
    init: function (a) {
        this._super(a);
        this.panelId = this.jqId + "_panel";
        this.input = $(this.jqId + "_input");
        ...
        var a = this;
        if (IChartsFaces.env.browser.webkit) {
        ...
        if (this.cfg.filter) {
            ...
            IChartsFaces.skinInput(this.filterInput);
                    ...
                    if (!h) {
                        ...
                        if (c.length) {
                            ...
                            if (a.panel.is(":hidden")) {
                                ...
                                a.highlightItem(b);
                                IChartsFaces.scrollInView(a.itemsWrapper, b)
                            }
                        }
    ...
    highlightNext: function (b) {
        ...
        if (a.length === 1) {
            if (this.panel.is(":hidden")) {
                ...
                IChartsFaces.scrollInView(this.itemsWrapper, a)
            }
    ...
    show: function () {
        var a = this;
        this.alignPanel();
        this.panel.css("z-index", ++IChartsFaces.zindex);
        if ($.browser.msie && /^[6,7]\.[0-9]+/.test($.browser.version)) {
            this.panel.parent().css("z-index", IChartsFaces.zindex - 1)
        }
        if (this.cfg.effect !== "none") {
            this.panel.show(this.cfg.effect, {}, this.cfg.effectSpeed, function () {
                IChartsFaces.scrollInView(a.itemsWrapper, a.getActiveItem());
                if (a.cfg.filter) {
                    a.focusFilter()
                }
            })
            ...
            this.panel.show();
            IChartsFaces.scrollInView(this.itemsWrapper, this.getActiveItem());
            if (a.cfg.filter) {
                this.focusFilter(10)
            }
        }
...
IChartsFaces.widget.SelectOneRadio = IChartsFaces.widget.BaseWidget.extend({init: function (b) {
        this._super(b);
        if (this.cfg.custom) {
        ...
        this.bindEvents();
        this.inputs.data(IChartsFaces.CLIENT_ID_DATA, this.id)
    }, bindEvents: function () {
        var a = this;
        ...
        this.labels.filter(":not(.ui-state-disabled)").on("click.selectOneRadio", function (d) {
            var c = $(IChartsFaces.escapeClientId($(this).attr("for"))), b = null;
            if (c.is(":input")) {
                b = c.parent().next()
            } else {
...
IChartsFaces.widget.SelectBooleanCheckbox = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        this.input = $(this.jqId + "_input");
...
IChartsFaces.widget.SelectManyCheckbox = IChartsFaces.widget.BaseWidget.extend({init: function (b) {
        this._super(b);
        if (this.cfg.custom) {
...
IChartsFaces.widget.SelectListbox = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        ...
        if (b.length) {
            IChartsFaces.scrollInView(this.listContainer, this.items.eq(b.eq(0).index()))
        }
        this.bindEvents();
        this.input.data(IChartsFaces.CLIENT_ID_DATA, this.id)
    }, bindEvents: function () {
...
IChartsFaces.widget.SelectOneListbox = IChartsFaces.widget.SelectListbox.extend({bindEvents: function () {
        this._super();
        var a = this;
        if (!this.cfg.disabled) {
...
IChartsFaces.widget.SelectManyMenu = IChartsFaces.widget.SelectListbox.extend({bindEvents: function () {
        this._super();
        var a = this;
        if (!this.cfg.disabled) {
...
IChartsFaces.widget.CommandButton = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        IChartsFaces.skinButton(this.jq)
    }, disable: function () {
...
IChartsFaces.widget.Button = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        IChartsFaces.skinButton(this.jq)
    }, disable: function () {
        this.jq.removeClass("ui-state-hover ui-state-focus ui-state-active").addClass("ui-state-disabled").attr("disabled", "disabled")
...
IChartsFaces.widget.SelectManyButton = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        ...
        a.removeClass("ui-state-hover").addClass("ui-state-active").children(":checkbox").prop("checked", true).change()
    }, unselect: function (a) {
        a.removeClass("ui-state-active").addClass("ui-state-hover").children(":checkbox").prop("checked", false).change()
    }});
IChartsFaces.widget.SelectOneButton = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        this.buttons = this.jq.children("div:not(.ui-state-disabled)");
        ...
        a.addClass("ui-state-active").children(":radio").prop("checked", true).change()
    }});
IChartsFaces.widget.SelectBooleanButton = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        this.input = $(this.jqId + "_input");
        this.disabled = this.input.is(":disabled");
...
IChartsFaces.widget.SelectCheckboxMenu = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        ...
        $(IChartsFaces.escapeClientId(this.panelId)).remove();
        this.init(a)
        ...
        if (this.cfg.appendTo) {
            this.panel.appendTo(IChartsFaces.expressions.SearchExpressionFacade.resolveComponentsAsSelector(this.cfg.appendTo))
        } else {
        ...
        if (this.cfg.behaviors) {
            IChartsFaces.attachBehaviors(this.inputs, this.cfg.behaviors)
        }
    }, bindKeyEvents: function () {
        var c = this;
        ...
        this.panel.css({left: "", top: "", "z-index": ++IChartsFaces.zindex});
        ...
        a.on("focus.selectCheckboxMenu", function (f) {
            ...
            IChartsFaces.scrollInView(b.itemContainerWrapper, d)
...
IChartsFaces.widget.InputMask = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        if (this.cfg.mask) {
...
IChartsFaces.widget.Password = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        ...
        this.panel.css({left: "", top: "", "z-index": ++IChartsFaces.zindex}).position({my: "left top", at: "right top", of: this.jq})
    }, show: function () {
        if (!this.cfg.inline) {
            this.align();
            this.panel.fadeIn()
...
IChartsFaces.widget.DefaultCommand = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this.cfg = a;
        this.id = this.cfg.id;
        this.jqId = IChartsFaces.escapeClientId(this.id);
        this.jqTarget = $(IChartsFaces.escapeClientId(this.cfg.target));
        this.scope = this.cfg.scope ? $(IChartsFaces.escapeClientId(this.cfg.scope)) : null;
        var b = this;
...
IChartsFaces.widget.SplitButton = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        ...
        this.button.data(IChartsFaces.CLIENT_ID_DATA, this.id);
        this.menuButton.data(IChartsFaces.CLIENT_ID_DATA, this.id)
    }, refresh: function (a) {
        this.menu.remove();
        this.init(a)
    }, destroy: function () {
        ...
        var c = this;
        IChartsFaces.skinButton(this.button).skinButton(this.menuButton);
        ...
        this.menu.css({left: "", top: "", "z-index": ++IChartsFaces.zindex});
...
IChartsFaces.widget.ThemeSwitcher = IChartsFaces.widget.SelectOneMenu.extend({init: function (a) {
        this._super(a);
        var b = this;
        this.input.on("change", function () {
            IChartsFaces.changeTheme(b.getSelectedValue())
        })
    }});
IChartsFaces.widget.MultiSelectListbox = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        ...
        for (var a = (k.length - 1); a >= 0; a--) {
            ...
            if (b.is("li")) {
                ...
                if (b.is("ul")) {
                    ...
                    b.clone(true).appendTo(e).addClass("ui-multiselectlistbox-list").removeClass("ui-helper-hidden");
                    d.jq.append(e)
                }
            }
        }
...
IChartsFaces.widget.Growl = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this.cfg = a;
        this.id = this.cfg.id;
        this.jqId = IChartsFaces.escapeClientId(this.id);
        this.render();
...
IChartsFaces.widget.Inplace = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        ...
        if (!this.cfg.disabled) {
            ...
            if (this.cfg.editor) {
                ...
                IChartsFaces.skinButton(b).skinButton(d);
                b.click(function (f) {
                    c.save(f)
                });
                d.click(function (f) {
...
IChartsFaces.widget.LightBox = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        ...
        var c = this, b = IChartsFaces.env.ios ? "touchstart." + this.id : "click." + this.id, a = "resize." + this.id;
        ...
        this.center();
        this.panel.css("z-index", ++IChartsFaces.zindex).show();
...
IChartsFaces.widget.Menu = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        if (this.cfg.overlay) {
            this.initOverlay()
        }
        ...
        this.align();
        this.jq.css("z-index", ++IChartsFaces.zindex).show()
    }, hide: function () {
...
IChartsFaces.widget.TieredMenu = IChartsFaces.widget.Menu.extend({init: function (a) {
        this._super(a);
        ...
        a.css("z-index", ++IChartsFaces.zindex).show().position(c)
    }, reset: function () {
        var a = this;
        this.active = false;
...
IChartsFaces.widget.Menubar = IChartsFaces.widget.TieredMenu.extend({showSubmenu: function (b, a) {
        var c = null;
...
IChartsFaces.widget.SlideMenu = IChartsFaces.widget.Menu.extend({init: function (c) {
        this._super(c);
...
IChartsFaces.widget.PlainMenu = IChartsFaces.widget.Menu.extend({init: function (a) {
        this._super(a);
        ...
        IChartsFaces.setCookie(this.stateKey, this.collapsedIds.join(","))
    }, restoreState: function () {
        var b = IChartsFaces.getCookie(this.stateKey);
        if (b) {
            this.collapsedIds = b.split(",");
            for (var a = 0; a < this.collapsedIds.length; a++) {
                this.collapseSubmenu($(IChartsFaces.escapeClientId(this.collapsedIds[a])), false)
            }
        }
    }, clearState: function () {
        IChartsFaces.setCookie(this.stateKey, null)
    }});
IChartsFaces.widget.MenuButton = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        this.menuId = this.jqId + "_menu";
...
IChartsFaces.widget.ContextMenu = IChartsFaces.widget.TieredMenu.extend({init: function (b) {
        b.autoDisplay = true;
        this._super(b);
        ...
        this.jqTargetId = c ? document : IChartsFaces.escapeClientId(this.cfg.target);
        ...
        var a = this.jqTargetId + " tbody.ui-datatable-data > tr.ui-widget-content:not(.ui-datatable-empty-message)", b = this.cfg.event + ".datatable", c = this;
        $(document).off(b, a).on(b, a, null, function (h) {
            var f = IChartsFaces.widgets[c.cfg.targetWidgetVar], i = $(this);
            if (f.cfg.selectionMode && i.hasClass("ui-datatable-selectable")) {
                f.onRowRightClick(h, this, c.cfg.selectionMode);
                c.show(h)
            } else {
                if (f.cfg.editMode === "cell") {
                    var g = $(h.target), d = g.is("td.ui-editable-column") ? g : g.parents("td.ui-editable-column:first");
        ...
        $(document).off(c, b).on(c, b, null, function (d) {
            IChartsFaces.widgets[a.cfg.targetWidgetVar].onRowRightClick(d, $(this));
            a.show(d)
        })
    }, bindTree: function () {
        var b = this.jqTargetId + " .ui-tree-selectable", a = this.cfg.nodeType ? this.cfg.event + ".treenode." + this.cfg.nodeType : this.cfg.event + ".treenode", d = this.cfg.event + ".tree", c = this;
        $(document).off(a, b).on(a, b, null, function (g) {
            var f = $(this);
            if (c.cfg.nodeType === undefined || f.parent().data("nodetype") === c.cfg.nodeType) {
                IChartsFaces.widgets[c.cfg.targetWidgetVar].nodeRightClick(g, f);
                c.show(g)
            }
        });
        $(document).off(d, this.jqTargetId).on(d, this.jqTargetId, null, function (f) {
            if (IChartsFaces.widgets[c.cfg.targetWidgetVar].isEmpty()) {
                c.show(f)
            }
        })
    }, refresh: function (b) {
        var a = IChartsFaces.escapeClientId(b.id), c = $(a);
        if (c.length > 1) {
        ...
        var g = $(window), f = h.pageX, d = h.pageY, b = this.jq.outerWidth(), a = this.jq.outerHeight();
        ...
        this.jq.css({left: f, top: d, "z-index": ++IChartsFaces.zindex}).show();
        h.preventDefault();
        h.stopPropagation()
    }, hide: function () {
        var a = this;
...
IChartsFaces.widget.MegaMenu = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
...
IChartsFaces.widget.PanelMenu = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        ...
        a.find("> .ui-menuitem-link > .ui-panelmenu-icon").addClass("ui-icon-triangle-1-s");
        a.children(".ui-menu-list").show();
        if (!b) {
            this.addAsExpanded(a)
        }
    }, collapseTreeItem: function (a) {
        a.find("> .ui-menuitem-link > .ui-panelmenu-icon").removeClass("ui-icon-triangle-1-s");
        a.children(".ui-menu-list").hide();
        this.removeAsExpanded(a)
    }, saveState: function () {
        if (this.cfg.stateful) {
            var a = this.expandedNodes.join(",");
            IChartsFaces.setCookie(this.stateKey, a, {path: "/"})
        }
    }, restoreState: function () {
        var d = null;
        if (this.cfg.stateful) {
            d = IChartsFaces.getCookie(this.stateKey)
        }
        if (d) {
            this.collapseAll();
            this.expandedNodes = d.split(",");
            for (var c = 0; c < this.expandedNodes.length; c++) {
                var b = $(IChartsFaces.escapeClientId(this.expandedNodes[c]));
        ...
        if (this.cfg.stateful) {
            IChartsFaces.deleteCookie(this.stateKey, {path: "/"})
        }
...
IChartsFaces.widget.TabMenu = IChartsFaces.widget.Menu.extend({init: function (a) {
        this._super(a);
...
IChartsFaces.widget.Message = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        var b = this.jq.children(".ui-message-error-detail").text();
        if (b) {
            $(IChartsFaces.escapeClientId(this.cfg.target)).data("tooltip", b)
        }
    }});
IChartsFaces.widget.NotificationBar = IChartsFaces.widget.BaseWidget.extend({init: function (b) {
        this._super(b);
        var a = this;
...
IChartsFaces.widget.Panel = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
...
IChartsFaces.widget.Poll = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this.cfg = a;
        this.id = this.cfg.id;
        this.active = false;
...
IChartsFaces.widget.OrderList = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        ...
        var a = this;
        IChartsFaces.skinButton(this.jq.find(".ui-button"));
        ...
        d.each(function () {
            ...
            if (!e.is(":first-child")) {
                e.hide(b.cfg.effect, {}, "fast", function () {
                    e.insertBefore(e.prev()).show(b.cfg.effect, {}, "fast", function () {
                        a++;
                        if (c === a) {
                            b.saveState();
                            b.fireReorderEvent()
        ...
        d.each(function () {
            ...
            if (!e.is(":first-child")) {
                e.hide(b.cfg.effect, {}, "fast", function () {
                    e.prependTo(e.parent()).show(b.cfg.effect, {}, "fast", function () {
                        a++;
                        if (c === a) {
                            b.saveState();
                            b.fireReorderEvent()
        ...
        var b = this, d = $(this.items.filter(".ui-state-highlight").get().reverse()), c = d.length, a = 0;
        d.each(function () {
            var e = $(this);
            if (!e.is(":last-child")) {
                e.hide(b.cfg.effect, {}, "fast", function () {
                    e.insertAfter(e.next()).show(b.cfg.effect, {}, "fast", function () {
                        a++;
                        if (c === a) {
                            b.saveState();
                            b.fireReorderEvent()
        ...
        d.each(function () {
            ...
            if (!e.is(":last-child")) {
                e.hide(b.cfg.effect, {}, "fast", function () {
                    e.appendTo(e.parent()).show(b.cfg.effect, {}, "fast", function () {
                        a++;
                        if (c === a) {
                            b.saveState();
                            b.fireReorderEvent()
...
IChartsFaces.widget.OutputPanel = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
...
IChartsFaces.widget.OverlayPanel = IChartsFaces.widget.BaseWidget.extend({
    init: function (a) {
        this._super(a);
    ...
    align: function (e) {
        ...
        this.jq.css({left: "", top: "", "z-index": ++IChartsFaces.zindex}).position({my: this.cfg.my, at: this.cfg.at, of: document.getElementById(b), offset: a})
    },
    hide: function () {
        var a = this;
        if (this.cfg.hideEffect) {
...
IChartsFaces.widget.Paginator = IChartsFaces.widget.BaseWidget.extend({
    init: function (b) {
        this.cfg = b;
        this.jq = $();
        var a = this;
        $.each(this.cfg.id, function (c, d) {
            a.jq = a.jq.add($(IChartsFaces.escapeClientId(d)))
        });
        ...
        IChartsFaces.skinSelect(this.rppSelect);
        this.rppSelect.change(function (b) {
            if (!$(this).hasClass("ui-state-disabled")) {
                a.setRowsPerPage(parseInt($(this).val()))
            }
        });
        IChartsFaces.skinSelect(this.jtpSelect);
    ...
    updatePageLinks: function () {
        ...
        for (var e = a; e <= b; e++) {
            ...
            this.pagesContainer.append('<span class="' + f + '" tabindex="0">' + (e + 1) + "</span>")
        }
        if (c) {
...
IChartsFaces.widget.PickList = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        ...
        if (this.cfg.disabled) {
            ...
            var c = this, b = true;
            $(this.jqId + " ul").sortable({cancel: ".ui-state-disabled,.ui-chkbox-box", connectWith: this.jqId + " .ui-picklist-list", revert: 1, update: function (d, e) {
                    ...
                    c.fireTransferEvent(e.item, e.sender, e.item.parents("ul.ui-picklist-list:first"), "dragdrop")
                }, start: function (d, e) {
                    c.itemListName = c.getListName(e.item);
                    c.dragging = true
                }, stop: function (d, e) {
                    c.dragging = false
                }, beforeStop: function (d, e) {
                    if (c.itemListName !== c.getListName(e.item)) {
                        b = false
                    } else {
                        b = true
                    }
        ...
        this.items.on("mouseover.pickList", function (c) {
            ...
            if (!f.shiftKey) {
                ...
                if (a.cursorItem && (a.cursorItem.parent().is(l.parent()))) {
                    var h = l.index(), m = a.cursorItem.index(), k = (h > m) ? m : h, d = (h > m) ? (h + 1) : (m + 1), j = l.parent();
                    for (var c = k; c < d; c++) {
        ...
        b.children(".ui-picklist-item").each(function () {
            var e = $(this), f = IChartsFaces.escapeHTML(e.attr("data-item-value")), d = e.attr("data-item-label"), c = (d) ? IChartsFaces.escapeHTML(d) : "";
        ...
        var a = this;
        IChartsFaces.skinInput(this.sourceFilter);
        IChartsFaces.skinInput(this.targetFilter);
...
IChartsFaces.widget.ProgressBar = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
...
IChartsFaces.widget.Rating = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
...
IChartsFaces.widget.Resizable = IChartsFaces.widget.BaseWidget.extend({init: function (b) {
        this.cfg = b;
        this.id = this.cfg.id;
        this.jqId = IChartsFaces.escapeClientId(this.id);
        this.jqTarget = $(IChartsFaces.escapeClientId(this.cfg.target));
        ...
        if (this.cfg.isContainment) {
            this.cfg.containment = IChartsFaces.escapeClientId(this.cfg.parentComponentId)
        }
        var a = this;
...
IChartsFaces.widget.Slider = IChartsFaces.widget.BaseWidget.extend({init: function (b) {
        this._super(b);
        this.cfg.displayTemplate = this.cfg.displayTemplate || (this.cfg.range ? "{min} - {max}" : "{value}");
        if (this.cfg.range) {
            var a = this.cfg.input.split(",");
            this.input = $(IChartsFaces.escapeClientId(a[0]) + "," + IChartsFaces.escapeClientId(a[1]))
        } else {
            this.input = $(IChartsFaces.escapeClientId(this.cfg.input))
        }
        if (this.cfg.display) {
            this.output = $(IChartsFaces.escapeClientId(this.cfg.display))
        }
...
IChartsFaces.widget.Spinner = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
...
IChartsFaces.widget.Spotlight = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        this.target = IChartsFaces.expressions.SearchExpressionFacade.resolveComponentsAsSelector(this.cfg.target);
        ...
        var a = this;
        this.target.data("zindex", this.target.zIndex()).css("z-index", ++IChartsFaces.zindex);
...
IChartsFaces.widget.Sticky = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this.cfg = a;
        this.id = this.cfg.id;
        this.target = $(IChartsFaces.escapeClientId(this.cfg.target));
        ...
        if (!this.fixed || a) {
            this.target.css({position: "fixed", top: this.cfg.margin, "z-index": ++IChartsFaces.zindex}).addClass("ui-shadow ui-sticky");
...
IChartsFaces.widget.TabView = IChartsFaces.widget.DeferredWidget.extend({init: function (a) {
        this._super(a);
        ...
        for (var b = 0; b < this.navContainerItems.length; b++) {
            if (this.cfg.selected === b && !this.navContainerItems.eq(b).hasClass("ui-state-disabled")) {
                this.navContainerItems.eq(b).attr("tabindex", this.cfg.tabindex)
            } else {
        ...
        this.navContainer.children("li").on("focus.tabview", function () {
            ...
            if (!a.focusedTabHeader.hasClass("ui-state-disabled")) {
                a.navContainer.children('li[tabindex="' + a.cfg.tabindex + '"]').attr("tabindex", "-1").removeClass("ui-tabs-outline");
                a.focusedTabHeader.attr("tabindex", a.cfg.tabindex).addClass("ui-tabs-outline")
            }
        }).on("blur.tabview", function () {
            if (a.focusedTabHeader) {
                a.focusedTabHeader.removeClass("ui-tabs-outline")
            }
            ...
            switch (c.which) {
                ...
                case b.UP:
                    if (a.focusedTabHeader) {
                        if (a.cfg.scrollable && (a.focusedTabHeader.index() === 0)) {
                            break
                        }
                        if (a.focusedTabHeader.index() === 0) {
                            a.focusedTabHeader = a.navContainer.children("li:not(.ui-state-disabled):last")
                        } else {
                            a.focusedTabHeader = a.focusedTabHeader.prevAll("li:not(.ui-state-disabled):first");
                            if (!a.focusedTabHeader.length) {
                                a.focusedTabHeader = a.navContainer.children("li:not(.ui-state-disabled):last")
                            }
                        }
                        a.focusedTabHeader.trigger("focus.tabview");
                        if (a.cfg.scrollable) {
                            var d = a.focusedTabHeader.position().left < a.navcrollerLeft.position().left;
                            if (d) {
                                a.navcrollerLeft.trigger("click.tabview")
                            }
                        }
                    }
                    c.preventDefault();
                ...
                case b.DOWN:
                    if (a.focusedTabHeader) {
                        if (a.cfg.scrollable && (a.focusedTabHeader.index() === (a.getLength() - 1))) {
                            break
                        }
                        if (a.focusedTabHeader.index() === (a.getLength() - 1)) {
                            a.focusedTabHeader = a.navContainer.children("li:not(.ui-state-disabled):first")
                        } else {
                            a.focusedTabHeader = a.focusedTabHeader.nextAll("li:not(.ui-state-disabled):first");
                            if (!a.focusedTabHeader.length) {
                                a.focusedTabHeader = a.navContainer.children("li:not(.ui-state-disabled):first")
                            }
                        }
                        a.focusedTabHeader.trigger("focus.tabview");
                        if (a.cfg.scrollable) {
                            var f = a.focusedTabHeader.position().left + a.focusedTabHeader.width() > a.navcrollerRight.position().left;
                            if (f) {
                                a.navcrollerRight.trigger("click.tabview")
                            }
                        }
                    }
                    c.preventDefault();
        ...
        var d = this, c = a.index(), b = {source: this.id, process: this.id, update: this.id, params: [{name: this.id + "_contentLoad", value: true}, {name: this.id + "_newTab", value: a.attr("id")}, {name: this.id + "_tabindex", value: c}], onsuccess: function (h, f, g) {
                IChartsFaces.ajax.Response.handle(h, f, g, {widget: d, handle: function (i) {
                        a.html(i);
                        if (this.cfg.cache) {
                            this.markAsLoaded(a)
                        }
        ...
        var e = this.navContainer.children().eq(b), a = this.panelContainer.children().eq(b);
        e.remove();
        a.remove();
        ...
        if (d > 0) {
            if (b < this.cfg.selected) {
                ...
                if (b === this.cfg.selected) {
                    var c = (this.cfg.selected === (d)) ? (this.cfg.selected - 1) : this.cfg.selected;
                    this.select(c, true)
                }
            }
        } else {
...
IChartsFaces.widget.TagCloud = IChartsFaces.widget.BaseWidget.extend({init: function (b) {
        this._super(b);
        var a = this;
...
IChartsFaces.widget.Tooltip = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this.cfg = a;
        this.id = this.cfg.id;
        ...
        if (a.target) {
            if ($(IChartsFaces.escapeClientId(a.id)).length > 1) {
                $(document.body).children(IChartsFaces.escapeClientId(a.id)).remove()
            }
        } else {
        ...
        this.jq.css({left: "", top: "", "z-index": ++IChartsFaces.zindex});
        if (this.cfg.trackMouse && this.mouseEvent) {
            ...
            this.jq.position({my: "left top", at: "right bottom", of: this.target, collision: "flipfit"})
        }
    }, show: function () {
        if (this.target) {
            var a = this;
...
IChartsFaces.widget.BaseTree = IChartsFaces.widget.BaseWidget.extend({init: function (a) {
        this._super(a);
        ...
        if (this.cfg.dynamic) {
            ...
            if (b.data("processing")) {
                IChartsFaces.debug("Node is already being expanded, ignoring expand event.");
                return
            }
        ...
        if (!IChartsFaces.inArray(this.selections, a)) {
...
IChartsFaces.widget.VerticalTree = IChartsFaces.widget.BaseTree.extend({init: function (a) {
        this._super(a);
        ...
        b.draggable({helper: function () {
                ...
                d.height(20);
                return d
            }, appendTo: document.body, zIndex: ++IChartsFaces.zindex, revert: true, scope: a}).data({dragsourceid: this.jqId, dragmode: this.cfg.dragMode})
...
IChartsFaces.widget.HorizontalTree = IChartsFaces.widget.BaseTree.extend({init: function (a) {
        this._super(a);
        if (IChartsFaces.isIE()) {
            this.drawConnectors()
        }
    }, bindEvents: function () {
...
IChartsFaces.widget.TreeTable = IChartsFaces.widget.DeferredWidget.extend({init: function (a) {
        this._super(a);
        ...
        var e = this, b = {source: this.id, update: this.id, process: this.id, params: [{name: this.id + "_sorting", value: true}, {name: this.id + "_sortKey", value: d.attr("id")}, {name: this.id + "_sortDir", value: a}], onsuccess: function (h, f, g) {
                IChartsFaces.ajax.Response.handle(h, f, g, {widget: e, handle: function (i) {
                        this.tbody.html(i);
        ...
        var d = this, b = c.attr("data-rk"), a = {source: this.id, process: this.id, update: this.id, params: [{name: this.id + "_expand", value: b}], onsuccess: function (h, f, g) {
                IChartsFaces.ajax.Response.handle(h, f, g, {widget: d, handle: function (j) {
                        var i = c.next();
                        c.after(j);
        ...
        return IChartsFaces.inArray(this.selections, a)
    }, isSingleSelection: function () {
        return this.cfg.selectionMode == "single"
    }, isMultipleSelection: function () {
        return this.cfg.selectionMode == "multiple"
        ...
        return $.browser.webkit ? "15" : IChartsFaces.calculateScrollbarWidth()
    }, restoreScrollState: function () {
        ...
        if (d > 15 && f > 15) {
            ...
            var j = c.index();
            if (this.cfg.scrollable) {
                this.theadClone.find(IChartsFaces.escapeClientId(c.attr("id") + "_clone")).width(d);
                this.theadClone.find(IChartsFaces.escapeClientId(e.attr("id") + "_clone")).width(f);
                if (this.footerCols.length > 0) {
...
IChartsFaces.widget.Wizard = IChartsFaces.widget.BaseWidget.extend({init: function (b) {
        this._super(b);
        this.content = $(this.jqId + "_content");
        ...
        if (this.cfg.showNavBar) {
            ...
            IChartsFaces.skinButton(this.backNav);
            IChartsFaces.skinButton(this.nextNav);
