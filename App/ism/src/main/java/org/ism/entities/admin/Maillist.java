...
@NamedQueries({
    ...
    @NamedQuery(name = "Maillist.findUniqueMaillist", query = "SELECT m FROM Maillist m WHERE m.mlGroupe=:mlGroupe AND m.mlEvent = :mlEvent AND m.mlType = :mlType AND m.mlProcessus = :mlProcessus AND m.mlCompany = :mlCompany")
    })
public class Maillist implements Serializable {
    ...
    @Column(name = "ml_tos", nullable = true)
    private String mlTos;
    @Column(name = "ml_ccs", nullable = true)
    private String mlCcs;
    @Column(name = "ml_ccis", nullable = true)
