...
public class MaillistFacade extends AbstractFacade<Maillist> {
    ...
    private final String FIND_UNIQUE_MAILLIST = "Maillist.findUniqueMaillist";
    ...
    public Maillist findUnique(String mlGroupe, String mlEvent, IsmMailtype mlType, Processus processus, Company company) {
        em.flush();
        Query q = em.createNamedQuery(FIND_UNIQUE_MAILLIST)
                .setParameter("mlGroupe", mlGroupe)
                .setParameter("mlEvent", mlEvent)
                .setParameter("mlType", mlType)
                .setParameter("mlProcessus", processus)
                .setParameter("mlCompany", company);
        q.setMaxResults(1);
        List l = q.getResultList();
        if(l==null)
            return null;
        if(l.isEmpty())return null;
        return (Maillist) l.get(0);
    }
}
